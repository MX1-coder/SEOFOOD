    ...
    public function lddfw_edit_driver_service()
    {
        ...
        if ( isset( $_POST['lddfw_wpnonce'] ) ) {
            ...
            if ( !wp_verify_nonce( $nonce, 'lddfw-nonce' ) ) {
                $error = __( 'Security Check Failure - This alert may occur when you are logged in as an administrator and as a vendor on the same browser and the same device. If you want to work on both panels please try to work with two different browsers.', 'lddfw' );
            } else {
