...
class VI_WOO_COUPON_BOX_Admin_Export_Email {
	...
	public function generate_csv() {
		...
		if ( isset( $_POST['_woocouponboxexpemail_nonce'] ) && wp_verify_nonce( $_POST['_woocouponboxexpemail_nonce'], 'woocouponboxexpemail_action_nonce' ) ) { echo 'a1';
			$p_type        = 'wcb';
			$select_export = isset( $_POST['wcb_select_export'] ) ? sanitize_text_field( $_POST['wcb_select_export'] ) : 1;
			...
			if ( $select_export == '1' ) {
				...
				$wcb_post_campaign = isset( $_POST['wcb_post_campaign'] ) ? sanitize_text_field( $_POST['wcb_post_campaign'] ) : '';
...
//					ob_end_clean();
	...
	public function admin_enqueue_script() {
		...
		if ( $page == 'woo_coupon_box_export_email' ) {
			...
			// style
			wp_enqueue_style( 'woo-coupon-box-export-form', VI_WOO_COUPON_BOX_CSS . 'form.min.css' );
			wp_enqueue_style( 'woo-coupon-box-export-transition', VI_WOO_COUPON_BOX_CSS . 'transition.min.css' );
			wp_enqueue_style( 'woo-coupon-box-export-dropdown', VI_WOO_COUPON_BOX_CSS . 'dropdown.min.css' );
			wp_enqueue_style( 'woo-coupon-box-export-segment', VI_WOO_COUPON_BOX_CSS . 'segment.min.css' );
			wp_enqueue_style( 'woo-coupon-box-export-datepicker', VI_WOO_COUPON_BOX_CSS . 'wcb_datepicker.css' );
