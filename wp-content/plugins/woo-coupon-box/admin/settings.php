...
class VI_WOO_COUPON_BOX_Admin_Settings {
	...
	function preview_emails_html() {
		global $pagenow;
		if ( $pagenow == 'edit.php' && isset( $_REQUEST['post_type'] ) && isset( $_REQUEST['page'] ) && sanitize_text_field($_REQUEST['post_type']) === 'wcb' && sanitize_text_field($_REQUEST['page']) === 'woo_coupon_box' ) {
			?>
		...
		if ( $pagenow == 'edit.php' && isset( $_REQUEST['post_type'] ) && isset( $_REQUEST['page'] ) && sanitize_text_field($_REQUEST['post_type']) === 'wcb' && sanitize_text_field($_REQUEST['page']) === 'woo_coupon_box' ) {
			...
			if ( in_array( $editor_id, $editor_ids ) ) {
                ...
                <span class="button wcb-preview-emails-button"
                      data-wcb_language="<?php echo str_replace( 'wcb_email_content', '', $editor_id ) ?>"><?php esc_html_e( 'Preview emails', 'woo-coupon-box' ) ?></span>
				<?php
				echo ob_get_clean();
			}
		}
	...
	public function preview_emails_ajax() {
		...
		$button_shop_size     = isset( $_GET['button_shop_size'] ) ? sanitize_text_field( stripslashes( $_GET['button_shop_size'] ) ) : '';
		...
		$content         = str_replace( '{last_valid_date}', empty( $date_expires ) ? esc_html__( '', 'woo-coupon-box' ) : date_i18n( 'F d, Y', ( $date_expires - 86400 ) ), $content );
	...
	public function setting_page_woo_coupon_box() {
        ...
        <div class="wrap">
            ...
            <p class=""><?php _e('Please go to <a target="_blank" href="'.admin_url('admin.php?page=wc-settings&tab=email#woocommerce_email_base_color').'">WooCommerce Emails settings</a> to edit Email design','woo-coupon-box')?></p>
                                ...
                                <td>
                                    <div class="inline field">
                                        <input type="number" name="wcb_expire_subscribed" id="wcb_expire_subscribed"
                                               min="1"
                                               value="<?php echo $this->settings->get_params( 'wcb_expire_subscribed' ); ?>"> <?php esc_html_e( 'Days', 'woo-coupon-box' ); ?>
                                    </div>
                                    <label for="wcb_expire_subscribed"><span
                                                class="description"><?php esc_html_e( 'Show subscription form again after ', 'woo-coupon-box' ); ?>
                                            <span class="wcb_expire_subscribed_value"><?php echo $this->settings->get_params( 'wcb_expire_subscribed' ); ?></span><?php esc_html_e( ' days if the visitor subscribes', 'woo-coupon-box' ); ?></i></span></label>
                                </td>
                            </tr>
                            <tr valign="top">
                                <th scope="row">
                                ...
                                <td>
                                    <div class="vi-ui toggle checkbox checked">
                                        ...
                                        <label for="wcb_coupon_unique_free_shipping"><span
                                                    class="description"><?php printf( esc_html__( 'Enable if the coupon grants free shipping. A free shipping method must be enabled in your shipping zone and be set to require "a valid free shipping coupon" (see the "%s" setting).', 'woo-coupon-box' ), '<a href="https://docs.woocommerce.com/document/free-shipping/"
                                               target="_blank">' . esc_html__( 'free shipping method', 'woo-coupon-box' ) . '</a>' ) ?></span></label>
                                    </div>
                                </td>
                            </tr>
                            <tr valign="top" class="wcb-coupon-unique">
											...
											foreach ( $this->settings->get_params( 'wcb_coupon_unique_product_ids' ) as $product_id ) {
                                                ...
                                                <option value="<?php echo $product_id ?>" selected>
                                                    ...
                                                    if ($product){
                                                        ...
                                                        _e('Note*: Not found product(ID ='.$product_id.')', 'woo-coupon-box');
                                                    }
                                                    ?>
                                                </option>
												<?php
											...
											foreach ( $this->settings->get_params( 'wcb_coupon_unique_product_categories' ) as $category_id ) {
                                                ...
                                                <option value="<?php echo $category_id ?>" selected>
                                                    ...
                                                    if ($category){
                                                        ...
                                                        _e('Note*: Not found category(ID ='.$category_id.')', 'woo-coupon-box');
                                                    }
                                                    ?>
                                                </option>
												<?php
                    ...
                    <div class="vi-ui bottom attached tab segment wcb-container" data-tab="wcb-grecaptcha">
                        <table class="form-table">
                            ...
                            <tr align="top">
                                ...
                                <td>
                                    <select name="wcb_recaptcha_version" id="wcb_recaptcha_version" class="vi-ui fluid dropdown wcb_recaptcha_version">
                            ...
                            <tr align="top">
                                ...
                                <td>
                                    <input type="text" name="wcb_recaptcha_site_key" value="<?php echo $this->settings->get_params('wcb_recaptcha_site_key')?>">
                                </td>
                            </tr>
                            <tr align="top">
                                <th>
                                ...
                                <td>
                                    <input type="text" name="wcb_recaptcha_secret_key" value="<?php echo $this->settings->get_params('wcb_recaptcha_secret_key')?>">
                                </td>
                            </tr>
                            <tr align="top" class="wcb-recaptcha-v2-wrap" style="<?php echo $this->settings->get_params('wcb_recaptcha_version')==2?'':'display:none;'; ?>">
                                <th>
                                    <label for="wcb_recaptcha_secret_theme"><?php esc_html_e('Theme', 'woo-coupon-box') ?></label>
                                </th>
                                <td>
                                    <select name="wcb_recaptcha_secret_theme" id="wcb_recaptcha_secret_theme" class="vi-ui fluid dropdown wcb_recaptcha_secret_theme">
                            ...
                            <tr valign="top"  >
                                ...
                                <td>
                                    <div >
                                        <strong class="wcb-recaptcha-v2-wrap" style="<?php echo $this->settings->get_params('wcb_recaptcha_version')==2?'':'display:none;'; ?>">
                                            <?php esc_html_e('Get Google reCAPTCHA V2 Site and Secret key', 'woo-coupon-box') ?>
                                        </strong>
                                        <strong class="wcb-recaptcha-v3-wrap" style="<?php echo $this->settings->get_params('wcb_recaptcha_version')==3?'':'display:none;'; ?>">
                                            <?php esc_html_e('Get Google reCAPTCHA V3 Site and Secret key', 'woo-coupon-box') ?>
                                        </strong>
                                        <ul>
                                            <li><?php _e('1, Visit <a target="_blank" href="http://www.google.com/recaptcha/admin">page</a> to sign up for an API key pair with your Gmail account', 'woo-coupon-box') ?></li>

                                            <li class="wcb-recaptcha-v2-wrap" style="<?php echo $this->settings->get_params('wcb_recaptcha_version')==2?'':'display:none;'; ?>">
                                                <?php esc_html_e('2, Choose reCAPTCHA v2 checkbox ', 'woo-coupon-box') ?>
                                            </li>
                                            <li class="wcb-recaptcha-v3-wrap" style="<?php echo $this->settings->get_params('wcb_recaptcha_version')==3?'':'display:none;'; ?>">
	...
	public function admin_enqueue_script() {
		...
		if ( $page == 'woo_coupon_box' ) {
			...
			// style
			wp_enqueue_style( 'woo-coupon-box-form', VI_WOO_COUPON_BOX_CSS . 'form.min.css' );
			wp_enqueue_style( 'woo-coupon-box-button', VI_WOO_COUPON_BOX_CSS . 'button.min.css' );
			wp_enqueue_style( 'woo-coupon-box-dropdown', VI_WOO_COUPON_BOX_CSS . 'dropdown.min.css' );
			wp_enqueue_style( 'woo-coupon-box-checkbox', VI_WOO_COUPON_BOX_CSS . 'checkbox.min.css' );
			wp_enqueue_style( 'woo-coupon-box-icon', VI_WOO_COUPON_BOX_CSS . 'icon.min.css' );
			wp_enqueue_style( 'woo-coupon-box-transition', VI_WOO_COUPON_BOX_CSS . 'transition.min.css' );
			wp_enqueue_style( 'woo-coupon-box-tab', VI_WOO_COUPON_BOX_CSS . 'tab.css' );
			wp_enqueue_style( 'woo-coupon-box-segment', VI_WOO_COUPON_BOX_CSS . 'segment.min.css' );
			wp_enqueue_style( 'woo-coupon-box-menu', VI_WOO_COUPON_BOX_CSS . 'menu.min.css' );
			wp_enqueue_style( 'woo-coupon-box-select2', VI_WOO_COUPON_BOX_CSS . 'select2.min.css' );
			wp_enqueue_style( 'woo-coupon-box-admin', VI_WOO_COUPON_BOX_CSS . 'wcb-admin.css' );
			wp_enqueue_style( 'woocommerce-coupon-villatheme-support', VI_WOO_COUPON_BOX_CSS . 'villatheme-support.css' );
			...
			wp_enqueue_script( 'woo-coupon-box-form', VI_WOO_COUPON_BOX_JS . 'form.min.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-checkbox', VI_WOO_COUPON_BOX_JS . 'checkbox.min.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-dropdown', VI_WOO_COUPON_BOX_JS . 'dropdown.min.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-transition', VI_WOO_COUPON_BOX_JS . 'transition.min.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-tab', VI_WOO_COUPON_BOX_JS . 'tab.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-address', VI_WOO_COUPON_BOX_JS . 'jquery.address-1.6.min.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-select2', VI_WOO_COUPON_BOX_JS . 'select2.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_enqueue_script( 'woo-coupon-box-admin-javascript', VI_WOO_COUPON_BOX_JS . 'wcb-admin.js', array( 'jquery' ), VI_WOO_COUPON_BOX_VERSION );
			wp_localize_script( 'woo-coupon-box-admin-javascript', 'woo_coupon_box_params_admin', array(
		...
		if ( get_option( 'wcb_active' ) !== false ) {
			...
			foreach ( $args as $key => $arg ) {
				if ( in_array( $key, array(
					...
					$args[ $key ] = isset( $_POST[ $key ] ) ? wp_kses_post( stripslashes( $_POST[ $key ] ) ) : '';

				} else {
					$args[ $key ] = isset( $_POST[ $key ] ) ? sanitize_text_field( stripslashes( $_POST[ $key ] ) ) : '';
				}
			}
